...
async def download_instagram(url: str) -> str:
    try:
        ...
        media_id = instagrapi.extract_media_id_from_url(url)
        media = client.media_info(media_id)
        
        if media.media_type == 1:  # Rasm
            return media.dict()['carousel_media'][0]['image_versions2']['candidates'][0]['url']
        elif media.media_type == 2 or media.media_type == 8:  # Video yoki carousel
            return media.dict()['video_url']
